...
Formsy.Form = React.createClass({
  ...
  validate: function validate(component) {
    ...
    component.setState({
      ...
      $.data(document.body, 'validateForm', setTimeout((function() {
          this.validateForm();
      }).bind(this), 500));
    }).bind(this));
  },
  ...
  validateForm: function validateForm() {
    ...
    var onValidationComplete = (function () {
      allIsValid = inputKeys.every((function (name) {
        if (typeof inputs[name] != 'undefined') return inputs[name].state._isValid;
      }).bind(this));

      allIsValidWithoutRequire = inputKeys.every((function (name) {
        if (typeof inputs[name] != 'undefined') return inputs[name].state._isValidWithoutRequire;
      }).bind(this));
